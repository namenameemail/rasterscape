...
export const setActivePattern = (patternId: string | null) => (dispatch, getState: () => AppState) => {
    const prev = getState().activePattern.patternId;
    dispatch({type: EActivePatternAction.SET, patternId} as SetActivePatternAction);
    syncPatternsCook(prev, patternId);
};

export const resetActivePattern = () => (dispatch, getState: () => AppState) => {
    const prev = getState().activePattern.patternId;
    dispatch({type: EActivePatternAction.RESET, patternId: null} as SetActivePatternAction);
    syncPatternsCook(prev);
};
